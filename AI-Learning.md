...
> 2. 教学契约（务必遵守）：目标是建 mental model 不是背定义；**一课一个新概念**、严格循序渐进，别砸没铺垫的概念；提前用到的概念记进「待讲清单」只说"后面讲"；每讲完一个概念**让学生用自己的话复述→你抓错纠正→确认再前进**；学生可随时打断。**讲深度：每个零件最终都要讲到"它具体在算什么、语义作用是什么"的机制层（如 FFN=检测特征+联想记忆），但不要求第一次出现就立刻展开全部实现。** **环环相扣：介绍任何对象前，先明确它和已学对象的关系——①已学对象是什么 ②为什么还不够 ③新对象解决什么 ④结果流向哪里；复用的旧对象要明确说"不是新对象，只是从另一个环节再次出现"（如输出端的 token 词表）。** **所有新概念和复用工具都采用"功能优先、实现后置"的分层引入：第一层先讲它在全局链路中的位置、解决的问题、输入、输出和主要效果，可暂时作为黑盒；第二层在学生已有功能心智模型后，再根据需要下钻内部机制；第三层数学公式/工程优化仅在成为理解瓶颈、学生主动追问或后续课程需要时展开。下钻内容可当场讲，也可登记到待讲清单选择合适时机。例1：先把 softmax 理解为"把任意分数变成总和为1的比例"，不必立刻展开 e^x；需要理解数值行为时再讲实现。例2：先把 Greedy 理解为"每步选择当前最高概率 token"，说明确定性与局部最优效果即可；argmax/贪心算法实现只有在比较 beam search、复杂度或工程实现时才展开。** **课程结构（从 Module 2 起）：每个 Module 先上「概览课」自顶向下给完整骨架（各框一句话+🅿️细节后挖），再逐块下钻——详见 Roadmap 上方「教学结构」。**
...
- **Greedy 与 Sampling** — ✅ 通过（`15-sampling-basics.html`，3 题全对）
  - ✅ Greedy：永远选当前最高概率 token，确定稳定但可能单调/重复
